EESchema Schematic File Version 4
EELAYER 26 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title "Mock Pertelian"
Date "2021-10-11"
Rev "1.0"
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L MCU_Microchip_ATmega:ATmega168P-20PU U1
U 1 1 61641680
P 3600 3600
F 0 "U1" H 2960 3646 50  0000 R CNN
F 1 "ATmega168P-20PU" H 2960 3555 50  0000 R CNN
F 2 "Package_DIP:DIP-28_W7.62mm" H 3600 3600 50  0001 C CIN
F 3 "http://ww1.microchip.com/downloads/en/DeviceDoc/Atmel-8025-8-bit-AVR-Microcontroller-ATmega48P-88P-168P_Datasheet.pdf" H 3600 3600 50  0001 C CNN
	1    3600 3600
	1    0    0    -1  
$EndComp
$Comp
L Transistor_BJT:BC548 Q1
U 1 1 61641887
P 5250 5150
F 0 "Q1" H 5441 5196 50  0000 L CNN
F 1 "BC548" H 5441 5105 50  0000 L CNN
F 2 "Package_TO_SOT_THT:TO-92_Inline" H 5450 5075 50  0001 L CIN
F 3 "http://www.fairchildsemi.com/ds/BC/BC547.pdf" H 5250 5150 50  0001 L CNN
	1    5250 5150
	1    0    0    -1  
$EndComp
$Comp
L Device:Crystal Y1
U 1 1 61641C42
P 4800 3050
F 0 "Y1" V 4754 3181 50  0000 L CNN
F 1 "16MHz" V 4845 3181 50  0000 L CNN
F 2 "" H 4800 3050 50  0001 C CNN
F 3 "~" H 4800 3050 50  0001 C CNN
	1    4800 3050
	0    1    1    0   
$EndComp
Wire Wire Line
	4200 3000 4600 3000
Wire Wire Line
	4200 3100 4600 3100
Wire Wire Line
	4600 3000 4600 2900
Wire Wire Line
	4600 2900 4800 2900
Wire Wire Line
	4600 3100 4600 3200
Wire Wire Line
	4600 3200 4800 3200
$Comp
L Device:C_Small C2
U 1 1 61641FD5
P 5250 2900
F 0 "C2" V 5021 2900 50  0000 C CNN
F 1 "22p" V 5112 2900 50  0000 C CNN
F 2 "" H 5250 2900 50  0001 C CNN
F 3 "~" H 5250 2900 50  0001 C CNN
	1    5250 2900
	0    1    1    0   
$EndComp
$Comp
L Device:C_Small C3
U 1 1 61642042
P 5250 3200
F 0 "C3" V 5021 3200 50  0000 C CNN
F 1 "22p" V 5112 3200 50  0000 C CNN
F 2 "" H 5250 3200 50  0001 C CNN
F 3 "~" H 5250 3200 50  0001 C CNN
	1    5250 3200
	0    1    1    0   
$EndComp
Wire Wire Line
	4800 2900 5150 2900
Connection ~ 4800 2900
Wire Wire Line
	4800 3200 5150 3200
Connection ~ 4800 3200
$Comp
L power:GND #PWR06
U 1 1 61642159
P 5350 2900
F 0 "#PWR06" H 5350 2650 50  0001 C CNN
F 1 "GND" V 5355 2772 50  0000 R CNN
F 2 "" H 5350 2900 50  0001 C CNN
F 3 "" H 5350 2900 50  0001 C CNN
	1    5350 2900
	0    -1   -1   0   
$EndComp
$Comp
L power:GND #PWR07
U 1 1 61642184
P 5350 3200
F 0 "#PWR07" H 5350 2950 50  0001 C CNN
F 1 "GND" V 5355 3072 50  0000 R CNN
F 2 "" H 5350 3200 50  0001 C CNN
F 3 "" H 5350 3200 50  0001 C CNN
	1    5350 3200
	0    -1   -1   0   
$EndComp
$Comp
L Device:R_Small R2
U 1 1 616445E6
P 4950 5150
F 0 "R2" V 4754 5150 50  0000 C CNN
F 1 "1k" V 4845 5150 50  0000 C CNN
F 2 "" H 4950 5150 50  0001 C CNN
F 3 "~" H 4950 5150 50  0001 C CNN
	1    4950 5150
	0    1    1    0   
$EndComp
$Comp
L power:VCC #PWR08
U 1 1 6164499D
P 5350 4850
F 0 "#PWR08" H 5350 4700 50  0001 C CNN
F 1 "VCC" H 5367 5023 50  0000 C CNN
F 2 "" H 5350 4850 50  0001 C CNN
F 3 "" H 5350 4850 50  0001 C CNN
	1    5350 4850
	1    0    0    -1  
$EndComp
Wire Wire Line
	5350 4850 5350 4950
$Comp
L power:VCC #PWR01
U 1 1 61644A64
P 3600 2000
F 0 "#PWR01" H 3600 1850 50  0001 C CNN
F 1 "VCC" H 3617 2173 50  0000 C CNN
F 2 "" H 3600 2000 50  0001 C CNN
F 3 "" H 3600 2000 50  0001 C CNN
	1    3600 2000
	1    0    0    -1  
$EndComp
Wire Wire Line
	3600 2000 3600 2100
$Comp
L power:GND #PWR02
U 1 1 61644B42
P 3600 5250
F 0 "#PWR02" H 3600 5000 50  0001 C CNN
F 1 "GND" H 3605 5077 50  0000 C CNN
F 2 "" H 3600 5250 50  0001 C CNN
F 3 "" H 3600 5250 50  0001 C CNN
	1    3600 5250
	1    0    0    -1  
$EndComp
$Comp
L Device:R_Small R1
U 1 1 61644D9C
P 8050 4000
F 0 "R1" V 7854 4000 50  0000 C CNN
F 1 "10k" V 7945 4000 50  0000 C CNN
F 2 "" H 8050 4000 50  0001 C CNN
F 3 "~" H 8050 4000 50  0001 C CNN
	1    8050 4000
	1    0    0    -1  
$EndComp
$Comp
L power:VCC #PWR05
U 1 1 61645086
P 8050 3900
F 0 "#PWR05" H 8050 3750 50  0001 C CNN
F 1 "VCC" V 8067 4028 50  0000 L CNN
F 2 "" H 8050 3900 50  0001 C CNN
F 3 "" H 8050 3900 50  0001 C CNN
	1    8050 3900
	1    0    0    -1  
$EndComp
Wire Wire Line
	5350 5350 5350 5450
Wire Wire Line
	5350 5450 5450 5450
Text Label 5450 5450 0    50   ~ 0
A
Wire Wire Line
	3600 5100 3600 5250
NoConn ~ 4200 3400
NoConn ~ 4200 3500
NoConn ~ 4200 3600
NoConn ~ 4200 3700
Wire Wire Line
	3700 2100 3600 2100
Connection ~ 3600 2100
NoConn ~ 3000 2400
Text Label 4200 2900 0    50   ~ 0
D3
Text Label 4200 2800 0    50   ~ 0
D4
Text Label 4200 2700 0    50   ~ 0
D5
Text Label 4200 2600 0    50   ~ 0
D6
Text Label 4200 2500 0    50   ~ 0
D7
Text Label 4200 4800 0    50   ~ 0
D1
Text Label 4200 4700 0    50   ~ 0
D0
Text Label 4200 4500 0    50   ~ 0
E
Text Label 4200 4400 0    50   ~ 0
RS
Text Label 4200 4200 0    50   ~ 0
TXD
Text Label 4200 4100 0    50   ~ 0
RXD
NoConn ~ 4200 4600
NoConn ~ 4200 4300
Text Label 4200 2400 0    50   ~ 0
D2
$Comp
L Device:C_Small C1
U 1 1 61646776
P 4600 2300
F 0 "C1" H 4508 2254 50  0000 R CNN
F 1 "100n" H 4508 2345 50  0000 R CNN
F 2 "" H 4600 2300 50  0001 C CNN
F 3 "~" H 4600 2300 50  0001 C CNN
	1    4600 2300
	-1   0    0    1   
$EndComp
$Comp
L power:VCC #PWR03
U 1 1 616467D9
P 4600 2100
F 0 "#PWR03" H 4600 1950 50  0001 C CNN
F 1 "VCC" H 4617 2273 50  0000 C CNN
F 2 "" H 4600 2100 50  0001 C CNN
F 3 "" H 4600 2100 50  0001 C CNN
	1    4600 2100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR04
U 1 1 616467F4
P 4600 2500
F 0 "#PWR04" H 4600 2250 50  0001 C CNN
F 1 "GND" H 4605 2327 50  0000 C CNN
F 2 "" H 4600 2500 50  0001 C CNN
F 3 "" H 4600 2500 50  0001 C CNN
	1    4600 2500
	1    0    0    -1  
$EndComp
Wire Wire Line
	4600 2400 4600 2500
Wire Wire Line
	4600 2100 4600 2200
Wire Wire Line
	4200 3800 4850 3800
Text Label 5050 3800 0    50   ~ 0
TOUCH
$Comp
L Device:C_Small C4
U 1 1 61646F33
P 4850 4050
F 0 "C4" H 4758 4004 50  0000 R CNN
F 1 "22p" H 4758 4095 50  0000 R CNN
F 2 "" H 4850 4050 50  0001 C CNN
F 3 "~" H 4850 4050 50  0001 C CNN
	1    4850 4050
	-1   0    0    1   
$EndComp
$Comp
L Display_Character:WC1602A DS1
U 1 1 61647717
P 6850 3050
F 0 "DS1" H 6850 4028 50  0000 C CNN
F 1 "HD44780" H 6850 3937 50  0000 C CNN
F 2 "Display:WC1602A" H 6850 2150 50  0001 C CIN
F 3 "http://www.wincomlcd.com/pdf/WC1602A-SFYLYHTC06.pdf" H 7550 3050 50  0001 C CNN
	1    6850 3050
	1    0    0    -1  
$EndComp
Text Label 7250 2750 0    50   ~ 0
A
Wire Wire Line
	7250 2850 7350 2850
Wire Wire Line
	7350 2850 7350 2950
$Comp
L power:GND #PWR0101
U 1 1 61647B77
P 7350 2950
F 0 "#PWR0101" H 7350 2700 50  0001 C CNN
F 1 "GND" H 7355 2777 50  0000 C CNN
F 2 "" H 7350 2950 50  0001 C CNN
F 3 "" H 7350 2950 50  0001 C CNN
	1    7350 2950
	1    0    0    -1  
$EndComp
Wire Wire Line
	6850 3850 6850 3950
Wire Wire Line
	6850 3950 6950 3950
$Comp
L power:VCC #PWR0102
U 1 1 61647F3F
P 6950 3950
F 0 "#PWR0102" H 6950 3800 50  0001 C CNN
F 1 "VCC" V 6967 4078 50  0000 L CNN
F 2 "" H 6950 3950 50  0001 C CNN
F 3 "" H 6950 3950 50  0001 C CNN
	1    6950 3950
	0    1    1    0   
$EndComp
$Comp
L power:GND #PWR0103
U 1 1 61647F5E
P 4850 4300
F 0 "#PWR0103" H 4850 4050 50  0001 C CNN
F 1 "GND" H 4855 4127 50  0000 C CNN
F 2 "" H 4850 4300 50  0001 C CNN
F 3 "" H 4850 4300 50  0001 C CNN
	1    4850 4300
	1    0    0    -1  
$EndComp
Wire Wire Line
	7350 2250 7350 2450
Wire Wire Line
	6850 2250 7350 2250
Connection ~ 7350 2850
Wire Wire Line
	7250 2450 7350 2450
Connection ~ 7350 2450
Wire Wire Line
	7350 2450 7350 2850
Text Label 6450 3650 2    50   ~ 0
D7
Text Label 6450 3550 2    50   ~ 0
D6
Text Label 6450 3450 2    50   ~ 0
D5
Text Label 6450 3350 2    50   ~ 0
D4
Text Label 6450 3250 2    50   ~ 0
D3
Text Label 6450 3150 2    50   ~ 0
D2
Text Label 6450 3050 2    50   ~ 0
D1
Text Label 6450 2950 2    50   ~ 0
D0
Text Label 6450 2650 2    50   ~ 0
RS
Text Label 6450 2450 2    50   ~ 0
E
$Comp
L Connector:Conn_01x06_Male J1
U 1 1 61649DCA
P 6350 4650
F 0 "J1" H 6456 5028 50  0000 C CNN
F 1 "FTDI" H 6456 4937 50  0000 C CNN
F 2 "" H 6350 4650 50  0001 C CNN
F 3 "~" H 6350 4650 50  0001 C CNN
	1    6350 4650
	1    0    0    -1  
$EndComp
Wire Wire Line
	6550 4950 6650 4950
Wire Wire Line
	6650 4950 6650 5050
Wire Wire Line
	6550 4650 6700 4650
Wire Wire Line
	6550 4550 6700 4550
Text Label 6700 4650 0    50   ~ 0
TXD
Text Label 6700 4550 0    50   ~ 0
RXD
Text Label 8050 4250 2    50   ~ 0
RESET
Text Label 4200 3900 0    50   ~ 0
RESET
$Comp
L Device:C_Small C5
U 1 1 6164D7EC
P 7050 4450
F 0 "C5" V 6821 4450 50  0000 C CNN
F 1 "22p" V 6912 4450 50  0000 C CNN
F 2 "" H 7050 4450 50  0001 C CNN
F 3 "~" H 7050 4450 50  0001 C CNN
	1    7050 4450
	0    1    1    0   
$EndComp
Wire Wire Line
	6550 4450 6950 4450
Text Label 7150 4450 0    50   ~ 0
RESET
NoConn ~ 6550 4850
$Comp
L power:VCC #PWR0104
U 1 1 6164F8E6
P 6700 4750
F 0 "#PWR0104" H 6700 4600 50  0001 C CNN
F 1 "VCC" H 6718 4923 50  0000 C CNN
F 2 "" H 6700 4750 50  0001 C CNN
F 3 "" H 6700 4750 50  0001 C CNN
	1    6700 4750
	0    1    1    0   
$EndComp
Wire Wire Line
	6550 4750 6700 4750
$Comp
L power:GND #PWR0105
U 1 1 61652564
P 6650 5050
F 0 "#PWR0105" H 6650 4800 50  0001 C CNN
F 1 "GND" H 6655 4877 50  0000 C CNN
F 2 "" H 6650 5050 50  0001 C CNN
F 3 "" H 6650 5050 50  0001 C CNN
	1    6650 5050
	1    0    0    -1  
$EndComp
Wire Wire Line
	6850 2250 6250 2250
Wire Wire Line
	6250 2250 6250 2550
Wire Wire Line
	6250 2550 6450 2550
Connection ~ 6850 2250
Text Notes 6350 4450 2    50   ~ 0
DTR
Text Notes 6350 4550 2    50   ~ 0
TXO
Text Notes 6350 4650 2    50   ~ 0
RXI
Text Notes 6350 4750 2    50   ~ 0
VCC
Text Notes 6350 4850 2    50   ~ 0
CTS
Text Notes 6350 4950 2    50   ~ 0
GND
$Comp
L power:PWR_FLAG #FLG0101
U 1 1 61654375
P 6650 5050
F 0 "#FLG0101" H 6650 5125 50  0001 C CNN
F 1 "PWR_FLAG" V 6650 5178 50  0000 L CNN
F 2 "" H 6650 5050 50  0001 C CNN
F 3 "~" H 6650 5050 50  0001 C CNN
	1    6650 5050
	0    1    1    0   
$EndComp
Connection ~ 6650 5050
$Comp
L power:PWR_FLAG #FLG0102
U 1 1 616543C7
P 6700 4900
F 0 "#FLG0102" H 6700 4975 50  0001 C CNN
F 1 "PWR_FLAG" V 6700 5028 50  0000 L CNN
F 2 "" H 6700 4900 50  0001 C CNN
F 3 "~" H 6700 4900 50  0001 C CNN
	1    6700 4900
	0    1    1    0   
$EndComp
Wire Wire Line
	6700 4750 6700 4900
Connection ~ 6700 4750
$Comp
L power:PWR_FLAG #FLG0103
U 1 1 61654CAF
P 5350 5450
F 0 "#FLG0103" H 5350 5525 50  0001 C CNN
F 1 "PWR_FLAG" H 5350 5624 50  0000 C CNN
F 2 "" H 5350 5450 50  0001 C CNN
F 3 "~" H 5350 5450 50  0001 C CNN
	1    5350 5450
	-1   0    0    1   
$EndComp
Connection ~ 5350 5450
Text Label 4850 5150 2    50   ~ 0
BL
Text Label 4200 3300 0    50   ~ 0
BL
Wire Wire Line
	4850 3950 4850 3800
Connection ~ 4850 3800
Wire Wire Line
	4850 3800 5050 3800
Wire Wire Line
	4850 4300 4850 4150
Wire Wire Line
	8050 4250 8050 4100
$EndSCHEMATC
